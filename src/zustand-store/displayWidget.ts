...
export const useDisplayWidgetsStore = create<DisplayWidgetState>()(
    persist(
        (set, get) => ({
            ...
            targetId: undefined,
            replaceWidget: (targetId, newWidgetId) => set(() => {
                let displayWidgets = get().displayWidgets;
                const isItemAlreadyDisplayed = displayWidgets.some((widget) => widget.id == newWidgetId);

                if (!isItemAlreadyDisplayed) {
                    displayWidgets[targetId] = menuItems[newWidgetId]; // Replace the widget at targetId
